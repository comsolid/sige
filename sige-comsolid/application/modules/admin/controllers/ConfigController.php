   ...
   public function ajaxBuscarUsuariosAction() {
      ...
      try {
         $data = $model->buscarPermissaoUsuarios(
               ...
               $this->_request->getParam("buscar_por", "nome"),
      ...
      if ($this->getRequest()->isPost()) {
         ...
         if ($form->isValid($formData)) {
            ...
            try {
               ...
               $adapter->update("encontro_participante",
                       array('id_tipo_usuario' => $id_tipo_usuario),
                       "id_encontro = {$idEncontro} AND id_pessoa = {$id}");
               $adapter->commit();
               return $this->_helper->redirector->goToRoute(array(
                   'module' => 'admin',
                   'controller' => 'config',
