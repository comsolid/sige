...
class Application_Form_Pessoa extends Zend_Form
{
  ...
  {/*
    ...
    $usuario->addValidator('regex', false, array('/^[a-z]/i'));
    $usuario->setRequired(true);
    $usuario->setLabel('Usuário');

    $senha = new Zend_Form_Element_Text('senha');
    $senha->addValidator('regex', false, array('/^[a-z]/i'));
    $senha->setRequired(true);
    $senha->setLabel('Senha');

    $nome = new Zend_Form_Element_Text('nome');
    $nome->addValidator('regex', false, array('/^[a-z]/i'));
    $nome->setRequired(true);
    $nome->setLabel('Nome');

    $botaoOk = new Zend_Form_Element_Submit('OK');

    $this->setMethod("post");
    $this->setAction("/usuario");
    $this->addElement($nome);
    $this->addElement($usuario);
    $this->addElement($senha);
    $this->addElement($botaoOk);
    */
   
    $this->setAction('/participante/add')
           ->setMethod('post');
    ...
    $apelido->setRequired(true)
          ...
          ->addErrorMessage("Apelido deve ter no nímimo 6 caracteres");
       
   $sexo = $this->createElement('radio', 'id_sexo',array('label' => 'Sexo: '));
   $sexo->addMultiOptions(array('0' => 'Não Informado',
                                '1' => 'Masculino',
                                '2' => 'Feminino'))
        ->setRequired(true)
        ->setValue('0')
   ...
   $facebook->addValidator('EmailAddress')
            ->addErrorMessage("Facebook invalido");
